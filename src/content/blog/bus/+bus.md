...
This strange little project started one April afternoon in 2022 when I was waiting for the bus to take me home from school. For some context, my school is unfortunately located in a way that it usually takes a transfer across two buses to get home, even with Puget Sound's robust public transit system. This means that there are about 5 different routes I could take. Because the whole trip takes about 50 minutes on average, I'm always looking for the fastest, most optimal route.

So on this spring afternoon, I opened up Google Maps, clicked the fastest option, and walked to the corresponding bus stop. I expected the bus to arrive in a few minutes, because that's what Google Maps said. But 20 minutes later, it still had not arrived. I opened Google Maps again. The same bus was now somehow 10 minutes away. Frustrated, I ran to a different bus stop to try another route. Just as I arrived, the bus I was waiting for earlier passed by (even though it was still 6 minutes away on Google Maps). If only I had waited a few more minutes, I would have caught it. I ended up waiting another 15 minutes for the second bus, which was also late.

This infuriating experience, along with multiple others, led me to wonder: why are bus arrival times sometimes so inaccurate? I decided to look into how they created, and if there was a way to make them more accurate.

## transit apps and gtfs

Initially, I used Google Maps to get bus arrival times and plan my routes, but I later tried out some other apps including Onebusaway, Transit, and Moovit. Turns out, they all get their data from the same source: a [GTFS](https://gtfs.org/) (General Transit Feed Specification) feed. The G in GTFS initially stood for Google, who developed it in 2005 for use in the [Google Transit Trip Planner](https://googleblog.blogspot.com/2005/12/public-transit-via-google.html).
...
Both feeds contain optional fields for other information like vehicle bearing, occupancy status, etc. but unfortunately King County Metro does not provide this information. The other feeds are not really relevant to arrival time predictions, so I won't go into them. But it's pretty easy to see how an app like Google Maps would use this data to display predicted arrival times and bus locations.
...
While researching Onebusaway, I came across another open-source project called Transitime. Transitime is a piece of software that uses real-time GPS data to generate public transportation information, including bus arrival time predictions. It seems to be actively managed by the company [Swiftly](https://www.goswift.ly/), but there is also a somewhat maintained open-source version on [Github](https://github.com/TheTransitClock/transitime). I can't be sure if King County Metro uses Transitime either, but it should serve as a good example of how arrival time predictions are made.

Looking through the open source Transitime code, a word caught my eye: Kalman. The Kalman filter is a mathematical technique that uses a series of measurements observed over time to produce estimates of unknown variables. I won't pretend to understand the math behind it, but I can see how it is applied. When given new GPS positions of a bus, Transitime can use the bus's historical position data to predict its future position from the current position. This is how it can predict when a bus will arrive at a stop. It isn't as simple as interpolating a moving object along a path though. Buses stop (or "dwell") for varying amounts of time at stops, and this often won't be reflected by periodic GPS data. There are also other factors like layovers and schedule changes that can affect arrival times. Transitime attempts to take all of this into account to make accurate predictions. There are modules in the code that try to determine specific things like dwell.

It was at this point that I realized how difficult it is to make accurate arrival time predictions. There are so many factors that can affect a bus's arrival time, and it's impossible to predict the future.

## measuring prediction accuracy

I wanted to see how accurate the arrival time predictions were for King County Metro, so I wrote a few Python scripts that consumed the GTFS realtime data. This eventually ended up becoming the basis for my IB Math IA paper.

The first issue I ran into is that the actual arrival times are not provided in the GTFS realtime data. This is because GPS updates from buses come periodically. I'm not too sure what determines the frequency of these updates, but I think it's around every 30 seconds for King County Metro. With these sporadic updates, it is impossible to determine the exact time a bus arrives at a stop. However, buses would sometimes send a location update when they were at a stop. Thus, I decided to use physical location to determine when a bus arrived at a stop. I compared the latitude and longitude of the bus to the latitude and longitude of the stop. If the bus was within 30m of the stop, I considered the time of the location update to be the arrival time. This method is far from perfect, but it was the best I could do at the time (I later realized that Transitime has to deal with this exact issue, and they use a much better method of extrapolating the bus's position first to determine when it arrives at a stop).
...
It's a pretty fuzzy graph due to the sheer number of data points, but it does show a slight trend of predictions getting more accurate as the bus gets closer to the stop on the right side of the graph. It also shows the extent of inaccuracy in the predictions. Some of this is definitely due to issues with my methodology, but it's safe to assume that predictions made an hour before the bus arrives are not reliable at all.
...
However, I do think there are some ways. The fundamental reason Transitime and Onebusaway have to use so much interpolation is that buses only provide sporadic GPS updates. If buses could provide more frequent updates, it would be easier to determine when they arrive at stops, and thus make more accurate predictions. Additionally, if buses could provide exact stop arrival times (like with a sensor that detects when the bus doors open), predictions could be improved even further. This would certainly require costly hardware upgrades, but I believe it is possible.
...
Ultimately a major challenge in this field is the lack of funding and financial incentive. Projects like Onebusaway and Transitime have aging Java codebases that are slowly developed by a few dedicated people over time. Recently, the Open Software Transit Foundation, which runs Onebusaway, posted a [blog post](https://seattletransitblog.com/2023/10/17/onebusaway-needs-help/) asking for help. On the other hand, transit agencies are already struggling to provide reliable service, and investing in better arrival time predictions is certainly not a priority. However, I think that improving arrival time predictions could lead to more people using public transit, which would benefit the agencies in the long run. It's a bit of a catch-22.

As for me, this exploration into bus arrival time predictions has been an enlightening experience. I've learned a lot about how transit apps work, and the challenges that come with predicting bus arrival times. It's shifted my perspective on the complexity of public transit systems. I also gained some experience with Python and data analysis, which I think will be useful in the future.
